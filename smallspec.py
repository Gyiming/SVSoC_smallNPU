...
def main():
    ...
    with open("soc_configure.cfg","r+") as cfgfile:
        ...
        latency_npu1 = float(config.get("info","latency_npu1"))
        latency_npu2 = float(config.get("info","latency_npu2"))
        energy_npu1 = float(config.get("info","energy_npu1"))
        energy_npu2 = float(config.get("info","energy_npu2"))
    ...
    for i in range(1, frame+1):
        ...
        if (frames_checked == 0):
            if (i==1):
                ...
                pk,tk= schedule.laschdule(energy_budget,energy_npu1,energy_GPU,energy_DSP,energy_CPU1,energy_npu2,latency_npu1,latency_GPU,latency_DSP,latency_CPU1,latency_npu2,frames_predicted)
                if (t==1):
                    if (pk[t] == 'acc'):
        ...
        #need check
            ...
            if (i==1):
                ...
                accumulation_accelerator = accumulation_accelerator + accelerator_time[i] - start_time[i]
                #spec_soc_performance_first
                end_time_perf[i] = ISP_time[i] + latency_npu1 + latency_commit
                total_energy_spec_perf = total_energy_spec_perf + energy_npu1 + energy_commit
                NPU1_signal = end_time_perf[i] - latency_commit
                accumulation_spec_perf = end_time_perf[i] - start_time[i]
                #spec_soc_energy_first
                end_time_energy[i] = ISP_time[i] + latency_CPU5 + latency_commit
                total_energy_spec_energy = total_energy_spec_energy + energy_CPU5 + energy_commit
                ...
                if (ISP_time[i] > end_time_perf[i-1]):
                    ...
                    end_time_perf[i] = end_time_perf[i-1] + latency_npu1 + latency_commit
                    total_energy_spec_perf = total_energy_spec_perf + energy_npu1 + energy_commit
                    NPU1_signal = end_time_perf[i] - latency_commit
                    accumulation_spec_perf = accumulation_spec_perf + end_time_perf[i] - start_time[i]
                #spec_soc_energy_first
                ...
                if (i<=app_degree):
                        ...
                        if (t==1):
                            if (pk[t] == 'acc1'):
                                if (ISP_time[i] + latency_check > predict_time_p[i]):
                                    if (NPU1_signal > ISP_time[i] + latency_check):
                                        end_time_perf[i] = NPU1_signal + latency_npu1 + latency_commit
                                    else:
                                        end_time_perf[i] = ISP_time[i] + latency_npu1 +latency_check + latency_commit
                                    NPU1_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = predict_time_p[i]
                                    GPU_signal = predict_time_p[i]
                                    CPU_signal = predict_time_p[i]
                                    NPU2_signal = predict_time_p[i]
                                else:
                                    if (NPU1_signal > predict_time_p[i]):
                                        end_time_perf[i] = NPU1_signal + latency_npu1 + latency_commit
                                    else:
                                        end_time_perf[i] = predict_time_p[i] + latency_npu1 + latency_commit 
                                    NPU1_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = predict_time_p[i]
                                    GPU_signal = predict_time_p[i]
                                    CPU_signal = predict_time_p[i]
                            ...
                            if (pk[t] == 'acc2'):
                                if (ISP_time[i] + latency_check > predict_time_p[i]):
                                    end_time_perf[i] = ISP_time[i] + latency_npu2 + latency_check + latency_commit
                                    CPU_signal = predict_time_p[i]
                                    DSP_signal = predict_time_p[i]
                                    GPU_signal = predict_time_p[i]
                                    NPU1_signal = predict_time_p[i]
                                    NPU2_signal = end_time_perf[i] - latency_check - latency_commit
                                else:
                                    end_time_perf[i] = predict_time_p[i] + latency_npu2 + latency_commit
                                    CPU_signal = predict_time_p[i] 
                                    DSP_signal = predict_time_p[i]
                                    GPU_signal = predict_time_p[i]
                                    NPU1_signal = predict_time_p[i]
                                    NPU2_signal = predict_time_p[i] + latency_npu2
                                    total_energy_spec_perf = total_energy_spec_perf + energy_npu2 + energy_commit                                
                        else:
                            if (pk[t] == 'acc1'):
                                if (ISP_time[i] + latency_check > NPU1_signal):
                                    end_time_perf[i] = ISP_time[i] + latency_npu1 + latency_check + latency_commit
                                    NPU1_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = DSP_signal
                                    GPU_signal = GPU_signal
                                    CPU_signal = CPU_signal
                                    NPU2_signal = NPU2_signal
                                else:
                                    end_time_perf[i] = NPU1_signal + latency_npu1 + latency_commit
                                    NPU1_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = DSP_signal
                            ...
                            if (pk[t] == 'cpu'):
                                ...
                                total_energy_spec_perf = total_energy_spec_perf + energy_CPU1 + energy_commit   
                            if (pk[t] == 'acc2'):
                                if (ISP_time[i] + latency_check > NPU2_signal):
                                    end_time_perf[i] = ISP_time[i] + latency_npu2 + latency_check + latency_commit
                                    NPU2_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = DSP_signal
                                    GPU_signal = GPU_signal
                                    CPU_signal = CPU_signal
                                    NPU1_signal = NPU2_signal
                                else:
                                    end_time_perf[i] = NPU2_signal + latency_npu2 + latency_commit
                                    NPU2_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = DSP_signal
                                    GPU_signal = GPU_signal
                                    CPU_signal = CPU_signal
                    ...
                    if (i-predict_frame_location)==1 or (i-predict_frame_location)==2 or (i-predict_frame_location)==3:
                        ...
                        #check wrong
                            ...
                            if (t==1):
                                ...
                                if (pk[t] == 'acc2'):
                                    if (ISP_time[i] + latency_check > predict_time_p[i]):
                                        end_time_perf[i] = ISP_time[i] + latency_npu2 + latency_check + latency_commit
                                        NPU2_signal = ISP_time[i] + latency_npu2
                                        DSP_signal = predict_time_p[i]
                                        GPU_signal = predict_time_p[i]
                                        NPU1_signal = predict_time_p[i]
                                        CPU_signal = predict_time_p[i]                                
                        ...
                        #check correct
                            ...
                            if (t==1):
                                ...
                                if (pk[t] == 'cpu'):
                                    ...
                                    total_energy_spec_perf = total_energy_spec_perf + energy_CPU1 + energy_commit 
                                if (pk[t] == 'acc2'):
                                    end_time_perf[i] = NPU1_signal + latency_npu2 + latency_commit
                                    NPU2_signal = end_time_perf[i] - latency_commit
                                    DSP_signal = DSP_signal
                                    GPU_signal = GPU_signal
                                    CPU_signal = CPU_signal
                        ...
                        if (t==1):
                            if (pk[t] == 'acc1'):
                                if (NPU1_signal > predict_time_p[i]):
                                    end_time_perf[i] = NPU1_signal + latency_npu1
                                else:
                                    end_time_perf[i] = predict_time_p[i] + latency_npu1 + latency_commit 
                                NPU1_signal = end_time_perf[i] - latency_commit
                                DSP_signal = predict_time_p[i]
                            ...
                            if (pk[t] == 'cpu'):
                                ...
                                total_energy_spec_perf = total_energy_spec_perf + energy_CPU1 + energy_commit
                            if (pk[t] == 'acc2'):
                                end_time_perf[i] = NPU2_signal + latency_npu1 + latency_commit
                                NPU2_signal = end_time_perf[i] - latency_commit
                                DSP_signal = DSP_signal
                                GPU_signal = GPU_signal
                                CPU_signal = CPU_signal
